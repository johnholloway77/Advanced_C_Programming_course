...
Fptr fpArr[4] = {&add, &sub, &mult, &divi};

// performs the given operation on elements of the arrays
int *performOp(int *a, int *b, int size,
               /* ADD FUNCTION POINTER PARAMETER HERE */ Fptr fptr);
...
int main() {
  ...
  while (choice != 5) {
    ...
    result = performOp(array1, array2, size,
                       /* PASS IN FUNCTION POINTER FROM ARRAY[j] */ fpArr[j]);
